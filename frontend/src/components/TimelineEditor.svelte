<script lang="ts">
  ...
  import { exportEvents, createEvent, updateText, updateTime, transitionStatus, importEvents, assignEvent, addComment, getComments } from '../lib/collaboration/yjs-operations';
  import { lockEntry, unlockEntry, getLockHolder, getOnlineUsers } from '../lib/collaboration/awareness-lock';
  import { createShortcutManager, createDefaultShortcuts } from '../lib/shortcuts/shortcut-manager';
  import { recordEdit } from '../lib/collaboration/edit-history';
  ...
  let { doc, provider, userId, username, videoSrc, projectId, authToken }: Props = $props();
  ...
  // 视图与编辑状态
  let viewFilter = $state<'all' | 'mine'>('all'); // 「所有」/「只看自己」
  ...
  function observeEvents() {
    ...
    provider.awareness.on('change', () => { onlineUsers = getOnlineUsers(provider); });
    onlineUsers = getOnlineUsers(provider);
    updateFromYjs();
    return () => { yEvents.unobserveDeep(updateFromYjs); };
  }
  ...
  let filteredEvents = $derived.by<AssEvent[]>(() => {
    let list = viewFilter === 'mine'
      ? events.filter(e => e._assignedTo === userId)
      : events.slice();
    if (myRange) {
  ...
  function saveEdit() {
    ...
    if (realtimeSave) {
      updateText(doc, editingId, newText, userId);
    } else {
  ...
  function onStatusChange(e: AssEvent, status: EventStatus) {
    transitionStatus(doc, e.id, status, userId);
  }
  ...
  async function onImportAss(e: Event) {
    ...
    importEvents(doc, assDoc.events);
    input.value = '';
  }
  ...
  function setupShortcuts() {
    createDefaultShortcuts(shortcutManager, {
      ...
      deleteRow: () => { if (selectedId) { /* softDeleteEvent */ } },
      splitRow: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        ...
        const oldEnd = e.end;
        updateTime(doc, selectedId, e.start, mid, userId);
      ...
      nudgeStartBack: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        if (e && canEdit(e)) updateTime(doc, selectedId, Math.max(0, e.start - 0.1), e.end, userId);
      },
      nudgeStartForward: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        if (e && canEdit(e)) updateTime(doc, selectedId, Math.min(e.end - 0.1, e.start + 0.1), e.end, userId);
      },
      nudgeEndBack: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        if (e && canEdit(e)) updateTime(doc, selectedId, e.start, Math.max(e.start + 0.1, e.end - 0.1), userId);
      },
      nudgeEndForward: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        if (e && canEdit(e)) updateTime(doc, selectedId, e.start, e.end + 0.1, userId);
      },
      submitForReview: () => {
        if (!selectedId) return;
        const e = events.find(x => x.id === selectedId);
        if (e && canEdit(e)) transitionStatus(doc, selectedId, 'peer_review', userId);
      },
      jumpNextBlank: () => {
...
<svelte:window on:keydown={shortcutManager.handleKeyDown} />
  ...
  <div class="right-panel">
    ...
    <div class="toolbar">
      <div class="view-filter">
        <button class:active={viewFilter === 'all'} on:click={() => viewFilter = 'all'}>所有</button>
        <button class:active={viewFilter === 'mine'} on:click={() => viewFilter = 'mine'}>只看自己</button>
        ...
        <tbody>
          {#each filteredEvents as e, i (e.id)}
            ...
            <tr
              ...
              style="--status-color: {STATUS_COLORS[e._status]}"
